...
def load_mirna_targets(service_dir: str = None) -> dict:
    ...
    Returns a dict mapping miRNA IDs to all unique target genes.
    """
    ...
    try:
        if os.path.exists(mirna_file):
            ...
            # Group by miRNA and keep all unique target genes for each miRNA
            for mirna, group in df.groupby("miRNA"):
                targets = []
                seen_targets = set()
                for target_gene in group["Target Gene"]:
                    if isinstance(target_gene, str):
                        normalized_target = target_gene.strip()
                        if normalized_target and normalized_target not in seen_targets:
                            targets.append(normalized_target)
                            seen_targets.add(normalized_target)
                if targets:
                    mirna_map[mirna] = targets
...
def map_mirnas_to_genes(features: List[str], service_dir: str = None) -> List[str]:
    ...
    Returns list of genes with miRNAs replaced by all mapped target genes.
    """
    ...
    for feature in features:
        if isinstance(feature, str):
            ...
            if 'mir' in feature.lower():
                # Only exact miRNA ID matches are mapped.
                if feature in mirna_map:
                    mapped_features.extend(mirna_map[feature])
                else:
                    # If no exact match found, keep original miRNA feature
